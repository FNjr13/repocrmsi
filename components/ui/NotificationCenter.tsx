...
export default function NotificationCenter() {
  ...
  const [celebrationNotif, setCelebrationNotif] = useState<Notification | null>(null)
  ...
  const handleNotifClick = async (n: Notification) => {
    ...
    if (n.type === 'SALE_WON') {
      setOpen(false)
      setCelebrationNotif(n)
    }
  }

  // Extract lead name from SALE_WON notification title: "🏆 ¡Venta ganada! — {name}"
  const celebrationLeadName = celebrationNotif
    ? (celebrationNotif.title.split('— ')[1] ?? celebrationNotif.title)
    : ''
  ...
  return (
    <>
            ...
            <div className="max-h-[420px] overflow-y-auto">
              {loading && notifications.length === 0 ? (
                ...
                notifications.map(n => (
                  <div
                    key={n.id}
                    className={`flex gap-3 px-4 py-3 hover:bg-gray-50 transition-colors cursor-pointer border-b border-gray-50 ${!n.isRead ? 'bg-blue-50/40' : ''} ${n.type === 'SALE_WON' ? 'hover:bg-yellow-50/50' : ''}`}
      ...
      {/* SALE_WON celebration triggered from notification */}
      {celebrationNotif && (
        <WonCelebration
          leadName={celebrationLeadName}
          onClose={() => setCelebrationNotif(null)}
        />
      )}
    </>
  )
